...
User.all.each do |user|
  posts = [
            ...
            ["ジョガーパンツ","GU焼津店","GU","スキニーパンツを愛用していたのですが夏場は少し暑かったので今年は,ゆったり履け、足首が隠れる丈が欲しかったのでこちらを購入しました。","いわゆるトラックジャージで、少しキレイ目にも使える感じです。コーディネートしやすいです！","test_2"],
...
User.all.each do |user|
  posts = [
            ...
            ["はたらくきほん100","Amazon","松浦弥太郎 野尻哲也","仕事の基礎となる指針となる考えを探していました。「仕事をもっとやりがいのあるものにしたい！」と当時悩んでおり、手にとったのがこの本です。","1ページに1テーマ、仕事の指針についてまとめられています。自分が印象的だったのは「目の前の課題にどれだけ情熱を持てるかが大切です。」という一節です。今が見えなくなっていた自分を、ハッと気づかせてくれました。","test_2"],
...
User.all.each do |user|
  posts = [
            ...
            ["黒バックパック","メルカリ","MOLESKINE","通勤で使用するため耐水性のあるリュックを探していました。また、パソコンが入るようなポッケがあるものを選びました。","四角いフォルムが可愛く、通勤以外にも使っています。耐水性があるため土砂降りの中を傘をささずに行っても大丈夫です。（そのぐらい水を弾くという意味で実際は傘をさしてください）","test_3"]
          ]
  posts.shuffle.each do |title, place, brand, story, evidence,img|
  post = user.goods.new(
    {title: title,place: place,brand: brand,story: story,evidence: evidence}
