...
#   ["test2@test.com", 'ソロ', "test_2", password],
#   ["test3@test.com", 'ナギ', "test_3", password],
#   ["test4@test.com", 'ザンジ', "test_4", password],
#   ["test5@test.com", 'ショッパー', "test_5", password]
...
#   [
#     ["エアリズムTシャツ","UNIQLO渋谷店","UNIQLO","ダボッとした肌触りの良い服を探していました。友人に勧められ購入しました。","これ、イイです！表はコットンで裏はエアリズム。よくあるスポーツTシャツみたいにテロテロした生地感ではなく、表は本当にコットン似です。表裏違う生地を貼り合わせてある訳では無さそうなのに不思議です…。素晴らしいハイブリッド！","test_1"],
#     ["ジョガーパンツ","GU焼津店","GU","スキニーパンツを愛用していたのですが夏場は少し暑かったので今年は,ゆったり履けて足首が隠れる丈が欲しかったのでこちらを購入しました。","いわゆるトラックジャージの少しキレイ目にも使える感じです。コーディネートしやすいです！","test_2"],
...
#   ].each do |title, place, brand, story, evidence,img|
...
User.all.each do |user|
  [
    ["嫌われる勇気","Amazon","岸見一郎","周りの意見を気にしすぎて生きにくいと思っていた時に、Amazonでふと手にとってみました。","この本で印象的だった学びは「課題の分離」です。今まで相手の課題を解決しようと苦しんでいました。そこで課題を分離して自分の課題を集中することができました。","test_1"],
    ["はたらくきほん100","Amazon","松浦弥太郎 野尻哲也","仕事の基礎となる指針となるものを探していた。「仕事をもっとやりがいのあるものにしたい！」と当時悩んでおり、手にとったのがこの本です。","1ページに1テーマ、仕事の指針についてまとめられている。自分が印象的だったのは「目の前の課題にどれだけ情熱を持てるかが大切です。」という一節です。今が見えなくなっていた自分を、ハッと気づかせてくれました。","test_2"],
    ["君の膵臓を食べたい","戸田書店焼津店","住野よる","映画をみて原作も気になりました。映画で消化不十分になった分もっと理解したいと思い手にとってみました。","『偶然なんかじゃない。君が選択してきたんだよ』その言葉がすごく残った。2人のやり取りも、選ばれる言葉も、切ないぐらいに心地よい空気感。映画も良かったけど、この物語は、改めて本で読んで良かった。","test_3"]
  ].each do |title, place, brand, story, evidence,img|
...
User.all.each do |user|
  [
    ...
    ["黒腕時計","Amazon","G-shock","仕事で時間を測る機会があり、ストップウォッチ付きの腕時計を探していました。更に水に濡れても、強い衝撃を与えても大丈夫なものを選びました。","衝撃に強く、ストップウォッチ付きなので職場で大活躍しました。黒色のため普段のコーディネートにも最適です。","test_2"],
    ["黒バックパック","メルカリ","MOLESKINE","通勤で使用するため耐水性のあるリュックを探していました。また、パソコンが入るようなポッケがあるものを選びました。","四角いフォルムが可愛く、通勤以外にも使っています。耐水性があるため土砂降りの中を傘をささずに行っても大丈夫です。","test_3"]
  ].each do |title, place, brand, story, evidence,img|
