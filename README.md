# :raised_hands: Mybest
Mybestは**自分のモノに対する判断軸**を作るための、所有物を管理、共有するサービスです。  
衣類、本、雑貨の3分類が登録できますが、登録数はそれぞれ制限があります。  
登録する過程で本当に自分に**必要なモノ、好きなモノ**を探しましょう!  

<img width="800" alt="スクリーンショット 2020-11-19 17 04 43" src="https://user-images.githubusercontent.com/67394974/99638355-719e0680-2a89-11eb-85db-b1d8462cf843.png">

# :globe_with_meridians: URL
URL: https://mybst.info/

※トップページの中央のログインボタンをクリックしログイン画面に遷移した後、
「ゲストログイン(閲覧用)」のボタンをクリックすると、フォームに文字を入力せずにログインできます。


# :file_folder:制作の背景

私は断捨離が趣味です。

そんな私ですが、1年前はいわゆる汚部屋の住人でした。  
しかし、今は生活にミニマリズムを取り入れ、自分の好きなモノに囲まれて過ごしています。  

【現在の自分の部屋】
<img width="800" src="https://user-images.githubusercontent.com/67394974/99733961-55d84600-2b05-11eb-99a7-45dc39efd0c3.jpeg">

断捨離ができなかった頃の自分の課題は以下の通りでした。  

* __「いつか使うだろう」と捨てることを後回しにしてしまう__
* __モノを捨てるが続かない、すぐに元通りになってしまう__
* __「セールだ！」とモノを余分に買い込んでしまう__　

なぜ断捨離ができなかったのか？  

**それは、「何を残すか」ということを決めていなかったからです。**  
つまり、自分の最適なモノの**判断軸**がフワフワしたままになっていました。  
また、**判断軸**が明確になった時、「こだわって選んだモノを皆に見せたい！」と感じました。  
以上の経験を踏まえ、**「物に対する判断軸を見つけ、残したお気に入りのモノを共有する」サービス**があれば  
上記の課題を解決できるのではないかと思い、本アプリを制作しました。
  
# :relaxed:アプリを利用するメリット
このアプリを使用することで以下のメリットが得られます。

### モノが減る
* モノを探す時間、管理する時間が減り、**本当にやりたいことに時間を費やせる**
* 意思決定の機会が減り、**集中力が上がる**
* 維持コストや無駄な出費が減り、**節約になる**

### モノの減らし方がわかる
* 自分のモノに対する**判断軸が見つかる**
* **お気に入りのモノに囲まれて生活**できる

# :dizzy:工夫した点
* モノに対する判断軸を決めるために、あえて**投稿の制限**を付けました。
* 「モノをなぜ残すのか」を考えるために、**質問形式**の投稿フォームにしました。
* 人によってわずかに枠をオーバーしてしまう可能性があるため、制限付きで**増枠**できるようにしました。（max10枠まで）
* 「**直感的なわかりやすさ・使いやすさ**」 を重視し、一つ一つの機能やデザインに細部までこだわりました。
* **ミニマリズム**を意識するために、色は3色以内で余白を取ったデザインにしました。

<img width="400" alt="スクリーンショット 2020-11-19 16 28 36" src="https://user-images.githubusercontent.com/67394974/99635901-056dd380-2a86-11eb-9bae-9d6ff9a6d8dd.png">  <img width="400" alt="スクリーンショット 2020-11-19 16 29 41" src="https://user-images.githubusercontent.com/67394974/99636315-9c3a9000-2a86-11eb-9620-bc58b3af4d12.png">


# :green_book:機能一覧
### ユーザー管理機能
* ユーザー登録・編集・削除（devise）
* ゲストログイン
* プロフィール画像の登録・編集（Active Starage)
* マイページに以下の投投稿の一覧表示
  * すべての自分の投稿
  * 自分がピックアップした投稿
  
### 投稿機能
* 衣類、本、雑貨の投稿、編集、削除
* 投稿数制限・投稿数拡張
* 投稿画像のプレビュー表示
* 一覧表示・詳細表示
* ページネーション（kaminari）
* タグ付け・タグ検索(acts-as-taggable-on)
* いいね付け・いいね数に応じてトレンド一覧表示

### コメント機能
* 投稿に対するコメント、コメント削除

### ピックアップ機能
* 他人の投稿を保存
* 保存した投稿をマイページに一覧表示

### フォロー機能
* ユーザーのフォロー・フォロー解除
* フォロー中のユーザーとフォロワーの一覧表示

# :orange_book:使用技術
### フロントエンド
* Haml
* Scss
* JavaScript

### バックエンド
* Ruby 2.6.5
* Rails 6.0.3.4

### データベース
* MySQL2

### テスト
* RSpec
* EactoryBot
* Faker

### 開発環境
* Docker
* RuboCop

### 本番環境
* AWS（VPC、EC2、S3、Route53、ACM、ALB）
* Nginx、Unicorn、MariaDB

# :cloud:インフラ構成図  
<img width="800" alt="スクリーンショット 2020-11-20 7 49 43" src="https://user-images.githubusercontent.com/67394974/99733802-1578c800-2b05-11eb-9aed-a9fee6f7546c.png">


# :scroll:ER図

